...
import './index.scss';
const VirList: FC<White.VirListProps> = ({
  list = [],
  itemH,
  itemRender,
  wrapNum = 1,
  ...
  const [virObject, setVirObject] = useState<{
    data: any[];
    transform: number;
  }>({ data: itemList.current, transform: 0 });
  ...
  const toMoveWapper = useCallback(() => {
    ...
    const viewData = itemList.current.slice(
      (!start ? start : start - 1) * wrapNum,
      (end === itemList.current.length ? end : end + 1) * wrapNum,
    );
    setVirObject({
      data: viewData,
  ...
  }, [showLoadMore, page, list, pageSize, toMoveWapper]);
  ...
  const handleScroll = useThrottle(
    () => {
      ...
      const loadMore =
        scrollTop >= totalHeight() - wrapperRef.current.clientHeight;
    ...
    0,
    [showLoadMore, wrapperRef, totalHeight, onRefresh, toMoveWapper],
  );

  // 移动监听
  const handleTouchMove = useThrottle(
    (pageY: number) => {
    ...
    16.5,
    [moveStart, virObject, refreshHeight, wrapperRef],
  );
  // 移动开始
  ...
  // 移动结束清空
  const handleEndMove = useDebounce(
    async () => {
    ...
    16.5,
    [virObject, moveLast, moveStart, refreshHeight, handleRefresh, wrapperRef],
  );
  const isRefresh = onRefresh.current;
  return (
    <div
      className={`vir_list ${wrapperClass}`}
    ...
    >
      ...
      />
      <div
        className={`view-content ${wrapNum > 1 && 'row_wrap'}`}
        style={{
          transform: `translate3d(0, ${virObject.transform}px, 0)`,
          transition: isRefresh ? 'all 0.0065s' : 'none',
          marginTop: -refreshHeight,
        }}
      >
        <div
          className="refresh_view row_center"
          style={{ height: refreshHeight, opacity: isRefresh ? 1 : 0 }}
        >
          {
        ...
        >
          {isEndLoad || list.length === itemList.current.length ? (
            '----到底了----'
          ) : (
            <LoadingView height={24} width={24} />
          )}
